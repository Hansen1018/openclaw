...
private func outboxTestCommand(
    id: String,
    text: String,
...
private func seedOutboxBranch(
    _ store: OpenClawChatSQLiteTranscriptCache,
    sessionKey: String = "main",
    agentID: String? = nil) async -> Bool
{
...
private actor OutboxTransportState {
    ...
    var branchListGate: DeleteGate?
    let branchListStarted = DeleteGate()
    ...
    func setBranchListGate(_ gate: DeleteGate?) {
        self.branchListGate = gate
    }

    func waitUntilBranchListStarted() async {
        await self.branchListStarted.wait()
    }

    func awaitBranchListGate() async {
        await self.branchListStarted.open()
        if let branchListGate {
            await branchListGate.wait()
        }
    }
    ...
    func setBranchSwitchFails(_ fails: Bool) {
        self.branchSwitchFails = fails
    }

    func recordBranchListRequest(sessionKey: String, agentID: String?) {
        self.branchListRequestCount += 1
        self.branchListRequests.append((sessionKey, agentID))
    }
...
private final class OutboxTestTransport: @unchecked Sendable, OpenClawChatTransport {
    ...
    init(
        ...
        activeBranchLeafEntryID: String? = "leaf-new",
    ...
    {
        await self.state.recordBranchListRequest(sessionKey: sessionKey, agentID: agentID)
        await self.state.awaitBranchListGate()
        if !self.supportsBranchListing {
            throw GatewayResponseError(
    ...
    func switchSessionBranch(sessionKey _: String, leafEntryId: String) async throws {
        ...
        guard await !self.state.branchSwitchFails else { throw OutboxSendError() }
    }
...
private func sendWhileOffline(_ vm: OpenClawChatViewModel, text: String) async throws {
    try await waitUntil("active branch metadata") {
        await MainActor.run { vm.sessionBranches.contains(where: \.active) }
    }
    await MainActor.run {
        vm.input = text
        vm.send()
...
private protocol BranchScopeForwardingOutbox: OpenClawChatCommandOutbox {
    var branchStateBase: OpenClawChatSQLiteTranscriptCache { get }
}

extension BranchScopeForwardingOutbox {
    func branchState(for scope: OpenClawChatOutboxScope) async -> OpenClawChatOutboxBranchState? {
        await self.branchStateBase.branchState(for: scope)
    }

    func beginBranchSwitch(_ scope: OpenClawChatOutboxScope) async -> Bool {
        await self.branchStateBase.beginBranchSwitch(scope)
    }

    func cancelBranchSwitch(_ scope: OpenClawChatOutboxScope) async -> Bool {
        await self.branchStateBase.cancelBranchSwitch(scope)
    }
    ...
    {
        await self.branchStateBase.reconcileBranchScope(
            scope,
    ...
    func confirmBranchChange(
        ...
        lastError: String) async -> [OpenClawChatOutboxCommand]?
    {
        await self.branchStateBase.confirmBranchChange(
            scope,
            activeLeafEntryID: activeLeafEntryID,
            lastError: lastError)
    }
...
private actor DelayingOutbox: OpenClawChatCommandOutbox {
    ...
    private var supersedeNextRetry = false
    private var claimRelease: DeleteGate?
    private let enqueueStarted = DeleteGate()
    private let recoveryAttempted = DeleteGate()
    private let claimStarted = DeleteGate()
    ...
    func supersedeNextRetryAttempt() {
        self.supersedeNextRetry = true
    }

    func holdNextClaim() {
        self.claimRelease = DeleteGate()
    }

    func waitUntilClaimed() async {
        await self.claimStarted.wait()
    }

    func releaseClaim() async {
        await self.claimRelease?.open()
        self.claimRelease = nil
    }
    ...
    func claimNextCommand() async -> OpenClawChatOutboxCommand? {
        let command = await self.base.claimNextCommand()
        if let claimRelease {
            await self.claimStarted.open()
            await claimRelease.wait()
        }
        return command
    }
    ...
    func confirmBranchChange(
        ...
        lastError: String) async -> [OpenClawChatOutboxCommand]?
    {
        await self.base.confirmBranchChange(
            scope,
            activeLeafEntryID: activeLeafEntryID,
            lastError: lastError)
    }
    ...
    {
        if self.supersedeNextRetry {
            self.supersedeNextRetry = false
            _ = await self.base.confirmBranchChange(
                OpenClawChatOutboxScope(sessionKey: "main", agentID: agentID),
                activeLeafEntryID: "leaf-retry-conflict",
                lastError: "Branch changed during retry.")
        }
        return await self.base.markCommandRetriedIfPresent(
            id: id,
...
private actor SnapshotHoldingOutbox: BranchScopeForwardingOutbox {
    private nonisolated let base: OpenClawChatSQLiteTranscriptCache
    private var captured = DeleteGate()
    private var release = DeleteGate()
    private var shouldHoldNextLoad = false
    private var shouldHoldAllLoads = false

    init(base: OpenClawChatSQLiteTranscriptCache) {
        self.base = base
    }

    nonisolated var branchStateBase: OpenClawChatSQLiteTranscriptCache {
        self.base
    }
    ...
    func holdLoadsUntilReleased() {
        self.captured = DeleteGate()
        self.release = DeleteGate()
        self.shouldHoldAllLoads = true
    }

    func releaseSnapshot() async {
        self.shouldHoldAllLoads = false
    ...
    func loadCommands() async -> [OpenClawChatOutboxCommand] {
        ...
        if self.shouldHoldNextLoad || self.shouldHoldAllLoads {
...
private actor CancellationHoldingOutbox: BranchScopeForwardingOutbox {
    private nonisolated let base: OpenClawChatSQLiteTranscriptCache
...
struct ChatViewModelOutboxTests {
    @Test func `offline send queues durably and renders queued row`() async throws {
        ...
        #expect(commands.map(\.branchEpoch) == [0])
    ...
    @Test func `failed branch switch clears the durable lease`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        ...
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            ...
            retryCount: 0,
            lastError: nil)))
        #expect(await store.claimNextCommand()?.id == "after-failure")
    }

    @Test func `transient branch list failure leaves rows queued and replay paused`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        ...
        await MainActor.run { vm.load() }
        try await waitUntil("branch reconcile attempted") {
    ...
    @Test func `branch reconcile retries past three attempts and resumes delivery`() async throws {
        ...
        try await waitUntil("reconcile retry delivers") {
            await transport.state.sentMessages == ["send after retry"]
        }
        #expect(await MainActor.run { vm.reconciledOutboxBranchScopes.contains(scope) })
    }

    @Test func `healthy transition refreshes presentation branches`() async throws {
        let transport = OutboxTestTransport(healthy: true)
        let vm = await MainActor.run {
            OpenClawChatViewModel(sessionKey: "main", transport: transport)
        }
        await MainActor.run { vm.load() }
        try await waitUntil("initial healthy branch list") {
            let listed = await transport.state.branchListRequestCount > 0
            let healthy = await MainActor.run { vm.healthOK && vm.hasEstablishedTransportHealth }
            return listed && healthy
        }
        let initialCount = await transport.state.branchListRequestCount

        await MainActor.run {
            vm.applyTransportHealth(false)
            vm.applyTransportHealth(true)
        }

        try await waitUntil("branch list refresh after reconnect") {
            await transport.state.branchListRequestCount > initialCount
        }
    }

    @Test func `empty session reconciles root and delivers its first offline message`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        ...
        await MainActor.run { vm.load() }
        try await waitUntil("empty session outbox restore completes") {
            await MainActor.run { vm.hasRestoredOutboxMessages }
        }
        await MainActor.run {
        ...
        try await waitUntil("first message queues while offline") {
            await store.loadCommands().map(\.status) == [.queued]
        }

        await transport.goOnline()

        try await waitUntil("empty root scope reconciles and sends") {
            await transport.state.sentMessages == ["first message"]
        }
        #expect(await transport.state.branchListRequestCount > 0)
        #expect(await store.branchState(
            for: OpenClawChatOutboxScope(sessionKey: "main", agentID: "main"))?.epoch == 0)
    }

    @Test func `empty branch list parks rows from a previously nonempty scope`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let scope = OpenClawChatOutboxScope(sessionKey: "main", agentID: "main")
        ...
        try await waitUntil("empty listing parks the old transcript row") {
            await store.loadCommands().first?.status == .failed
        }
        #expect(await store.branchState(for: scope)?.epoch == 0)
        #expect(await store.branchState(for: scope)?.lastActiveLeafEntryID == nil)
        #expect(await transport.state.sentMessages.isEmpty)
    }

    @Test func `branch parking change stream refreshes a sibling view model`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.updateLastActiveLeafEntryID(
        ...
        let transport = OutboxTestTransport(
            healthy: false,
            activeBranchLeafEntryID: "leaf-old")
        ...
        try await waitUntil("both view models present the queued row") {
            await MainActor.run {
                initiatingVM.outboxStatesByMessageID.values.contains(.queued) &&
                    siblingVM.outboxStatesByMessageID.values.contains(.queued)
            }
        }

        let session = await MainActor.run { initiatingVM.currentSessionSnapshot() }
        #expect(await initiatingVM.confirmOutboxBranchChange(
            session,
    ...
    @Test func `matching epoch flush uses no branch list lookup`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let transport = OutboxTestTransport(healthy: true)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        await MainActor.run { vm.load() }
        try await waitUntil("branch scope reconciles") {
            await MainActor.run {
                vm.reconciledOutboxBranchScopes.contains(
                    OpenClawChatOutboxScope(sessionKey: "main", agentID: "main"))
            }
        }
        let branchListCount = await transport.state.branchListRequestCount
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            id: "local-epoch",
            sessionKey: "main",
            deliverySessionKey: "agent:main:main",
        ...
        try await waitUntil("matching epoch sends") {
            await transport.state.sentMessages == ["offline-safe"]
        }
        #expect(await transport.state.branchListRequestCount == branchListCount)
    }

    @Test func `reconnect invalidates branch reconciliation and reconciles again`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let scope = OpenClawChatOutboxScope(sessionKey: "main", agentID: "main")
        ...
        await MainActor.run {
            ...
            vm.isFlushingOutbox = true
            vm.reconcilePendingOutboxBranchScopes()
        }
        try await waitUntil("initial scope reconciliation") {
            await MainActor.run { vm.reconciledOutboxBranchScopes.contains(scope) }
        }
        let initialRequestCount = await transport.state.branchListRequestCount

        await MainActor.run {
            vm.applyTransportHealth(false)
            vm.readySessionMetadataGeneration = vm.sessionMetadataGeneration
        }
        #expect(await MainActor.run { vm.reconciledOutboxBranchScopes.isEmpty })
        await transport.state.setHealthy(true)
        await MainActor.run { vm.applyTransportHealth(true) }

        try await waitUntil("reconnect scope reconciliation") {
            let requestCount = await transport.state.branchListRequestCount
            let reconciled = await MainActor.run { vm.reconciledOutboxBranchScopes.contains(scope) }
            return requestCount > initialRequestCount && reconciled
        }
    }

    @Test func `bootstrap reconciles against leaf captured before history advances tip`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let scope = OpenClawChatOutboxScope(sessionKey: "main", agentID: "main")
        ...
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            ...
            routingContract: "per-sender|main|main",
            agentID: "main",
            text: "belongs to the old branch",
            thinking: "off",
            createdAt: Date().timeIntervalSince1970,
            status: .queued,
            retryCount: 0,
            lastError: nil)))
        let transport = OutboxTestTransport(healthy: true, activeBranchLeafEntryID: "leaf-new")
        let branchListGate = DeleteGate()
        await transport.state.setBranchListGate(branchListGate)
        await transport.state.setStaleHistoryRows([AnyCodable([
            "role": "assistant",
            "content": [["type": "text", "text": "new branch"]],
            "timestamp": 1.0,
            "__openclaw": ["id": "leaf-new"],
        ] as [String: Any])])
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        await MainActor.run { vm.load() }
        ...
        try await waitUntil("captured old leaf confirms branch switch") {
            let state = await store.branchState(for: scope)
            let status = await store.loadCommands().first?.status
            return state?.epoch == 1 && status == .failed
        }
    }

    @Test func `background branch reconciliation routes and parks by command agent`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        ...
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            ...
            routingContract: "per-sender|main|alpha",
            agentID: "alpha",
            text: "park alpha",
            thinking: "off",
            createdAt: now,
        ...
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            id: "beta-command",
            sessionKey: "global",
            deliverySessionKey: "agent:beta:main",
            routingContract: "per-sender|main|beta",
            agentID: "beta",
            text: "keep beta",
            thinking: "off",
            createdAt: now + 1,
        ...
        #expect(Set(requests.map { "\($0.sessionKey)|\($0.agentID ?? "")" }) == [
            "agent:alpha:main|alpha",
            "agent:beta:main|beta",
        ])
        let statuses = await Dictionary(
            uniqueKeysWithValues: store.loadCommands().map { ($0.id, $0.status) })
        #expect(statuses == ["alpha-command": .failed, "beta-command": .queued])
        #expect(await store.branchState(for: alphaScope)?.epoch == 1)
        #expect(await store.branchState(for: betaScope)?.epoch == 0)
    }

    @Test func `ambiguous bootstrap parks pending rows fail closed`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        let scope = OpenClawChatOutboxScope(sessionKey: "main", agentID: "main")
        ...
        try await waitUntil("ambiguous row parks") {
            await store.loadCommands().first?.status == .failed
        }
        #expect(await transport.state.sentMessages.isEmpty)
        #expect(await store.branchState(for: scope)?.epoch == 0)
        #expect(await store.branchState(for: scope)?.lastActiveLeafEntryID == "leaf-new")
    }

    @Test func `unsupported branching gateway leaves epoch zero rows deliverable`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            ...
            retryCount: 0,
            lastError: nil)))
        let transport = OutboxTestTransport(healthy: true, supportsBranchListing: false)
        let vm = await makeOutboxViewModel(transport: transport, outbox: store)
        await MainActor.run { vm.load() }

        try await waitUntil("unsupported branch gateway sends") {
            await transport.state.sentMessages == ["still flows"]
        }
        try await waitUntil("unsupported branch gateway confirms") {
            await store.loadCommands().isEmpty
        }
    }

    @Test func `presentation generation change requeues an abandoned claim`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.updateLastActiveLeafEntryID(
        ...
        await outbox.holdNextClaim()
        let transport = OutboxTestTransport(healthy: true)
        let vm = await makeOutboxViewModel(transport: transport, outbox: outbox)
        await MainActor.run { vm.load() }

        await outbox.waitUntilClaimed()
        await MainActor.run {
            vm.outboxPresentationGeneration &+= 1
            vm.healthOK = false
        }
        await outbox.releaseClaim()

        try await waitUntil("abandoned claim returns to the queue") {
            await store.loadCommands().first?.status == .queued
        }
        #expect(await transport.state.sentMessages.isEmpty)
    }

    @Test func `retry CAS miss reloads parked state without dropping transport health`() async throws {
        let url = try makeOutboxDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
        ...
        await MainActor.run { vm.load() }
        try await waitUntil("failed row and healthy transport are visible") {
            await MainActor.run {
                vm.healthOK && vm.messages.contains { vm.outboxState(for: $0.id)?.isFailed == true }
            }
        }
        let messageID = try #require(await MainActor.run {
            vm.messages.first { vm.outboxState(for: $0.id)?.isFailed == true }?.id
        })
        await outbox.supersedeNextRetryAttempt()
        await MainActor.run { vm.retryOutboxMessage(messageID) }

        try await waitUntil("superseding parked state reloads") {
            await MainActor.run {
                vm.outboxState(for: messageID) == .failed(reason: "Branch changed during retry.")
            }
        }
        #expect(await MainActor.run { vm.healthOK })
    ...
    @Test func `reconnect waits for canonical session metadata before flushing Ultra`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await seedOutboxBranch(store, agentID: "alpha"))
        #expect(await store.enqueueCommand(OpenClawChatOutboxCommand(
            id: "alpha-ultra",
            sessionKey: "main",
            deliverySessionKey: "agent:alpha:main",
    ...
    @Test func `reconnect retries metadata after a transient session list failure`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await seedOutboxBranch(store))
    ...
    @Test func `lagging history does not recursively refresh an acknowledged turn`() async throws {
        ...
        let historyRequestCount = await transport.state.historyRequestCount
        #expect(historyRequestCount <= 2, "history request count: \(historyRequestCount)")
    ...
    @Test func `flush gates captured thinking using the queued session metadata`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await seedOutboxBranch(store, sessionKey: "reasoning-session"))
        #expect(await seedOutboxBranch(store, sessionKey: "plain-session"))
        let now = Date().timeIntervalSince1970
        #expect(await store.enqueueCommand(
            OpenClawChatOutboxCommand(
                id: "c-think",
    ...
    @Test func `flushed background-session turn is spliced into its cached transcript`() async throws {
        ...
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-test")
        #expect(await seedOutboxBranch(store, sessionKey: "other-session"))
...
extension ChatViewModelOutboxTests {
    ...
    @Test func `send before restore adopts durable rows still queues behind them`() async throws {
        ...
        await outbox.holdLoadsUntilReleased()

        // Hold the restore snapshot so the send deterministically lands
        // before durable rows are adopted, even under parallel test load.
        let transport = OutboxTestTransport(healthy: true)
        let vm = await makeOutboxViewModel(transport: transport, outbox: outbox)
        await MainActor.run { vm.load() }
        ...
        try await waitUntil("instant send queues behind the held restore") {
            await store.loadCommands().count == 2
        }
