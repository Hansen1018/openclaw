...
struct ChatTranscriptCacheStoreTests {
    ...
    @Test func `v2 migration parks rows without a routing contract`() async throws {
        ...
        let migratedGlobal = try #require(commands.first { $0.id == "c-global" })
        #expect(await migrated.markCommandRetriedIfPresent(
            id: "c-global",
            expectedAttemptVersion: migratedGlobal.attemptVersion,
            expectedRetryCount: migratedGlobal.retryCount,
...
struct ChatCommandOutboxStoreTests {
    @Test func `outbox commands round trip in createdAt order across store instances`() async throws {
        ...
        #expect(loaded.map(\.attemptVersion) == [1, 1])
    ...
    @Test func `claims are FIFO and exclusive until the sender advances`() async throws {
        ...
        let firstClaim = try #require(await store.claimNextCommand())
        #expect(firstClaim.id == "c-1")
        #expect(await store.claimNextCommand() == nil)
        #expect(await store.markCommandAwaitingConfirmation(
            id: "c-1",
            attemptVersion: firstClaim.attemptVersion) == .updated)
    ...
    @Test func `user cancellation cannot delete a claimed command`() async throws {
        ...
        #expect(await store.confirmCommand(id: "c-1", attemptVersion: claim.attemptVersion) == .updated)
    ...
    @Test func `failed post-claim transition reopens same-process recovery`() async throws {
        ...
        #expect(await store.markCommandAwaitingConfirmation(
            id: "c-claim",
            attemptVersion: claim.attemptVersion) == .unavailable)
    ...
    @Test func `failed terminal transition reports unavailable and reopens recovery`() async throws {
        ...
        #expect(
            await store.markCommandFailedIfPresent(
                id: "c-terminal",
    ...
    @Test func `queued commands expire to failed at the staleness boundary`() async throws {
        ...
        #expect(await store.enqueueCommand(
            OpenClawChatOutboxCommand(
                ...
                retryCount: 0,
                lastError: nil)))
        #expect(await store.markCommandAwaitingConfirmation(id: "c-unconfirmed", attemptVersion: 1) == .updated)
    ...
    @Test func `outbox rows are scoped per gateway identity`() async throws {
        ...
        #expect(
            await storeB.markCommandFailedIfPresent(
                id: "c-a",
                attemptVersion: 1,
                retryCount: 3,
                lastError: "boom") == .missing)
    ...
    @Test func `branch observation preserves evidence until queued work reconciles`() async throws {
        ...
        #expect(await store.enqueueCommand(outboxCommand(
            ...
            sessionKey: "main",
            agentID: "agent-a",
            text: "do not move to B")))

        #expect(await !store.updateLastActiveLeafEntryID("leaf-b", expectedEpoch: 0, for: scope))
        let previousState = try #require(await store.branchState(for: scope))
        #expect(previousState.lastActiveLeafEntryID == "leaf-a")
        let reconciled = try #require(await store.reconcileBranchScope(
            scope,
            previousState: previousState,
            activeLeafEntryID: "leaf-b",
            branchLeafEntryIDs: ["leaf-a", "leaf-b"],
            lastError: "branch changed"))
        #expect(reconciled.first(where: { $0.id == "from-leaf-a" })?.status == .failed)
        #expect(await store.branchState(for: scope)?.lastActiveLeafEntryID == "leaf-b")

        #expect(await store.beginBranchSwitch(scope))
        #expect(await !store.updateLastActiveLeafEntryID("leaf-c", expectedEpoch: 1, for: scope))
        #expect(await store.branchState(for: scope)?.lastActiveLeafEntryID == "leaf-b")
        #expect(await store.cancelBranchSwitch(scope))
    }

    @Test func `retry after accepted or sending parking mints identity but queued keeps it`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        ...
        let sendingScope = OpenClawChatOutboxScope(sessionKey: "sending", agentID: "agent-a")
        #expect(await store.updateLastActiveLeafEntryID("leaf-a", expectedEpoch: 0, for: sendingScope))
        let sendingPreviousState = try #require(await store.branchState(for: sendingScope))
        #expect(await store.enqueueCommand(outboxCommand(
            id: "sending-id",
            sessionKey: "sending",
            agentID: "agent-a",
            text: "ack may be delayed")))
        #expect(await store.claimNextCommand()?.id == "sending-id")
        _ = try #require(await store.reconcileBranchScope(
            sendingScope,
            previousState: sendingPreviousState,
            activeLeafEntryID: "leaf-b",
        ...
        let queuedPreviousState = try #require(await store.branchState(for: queuedScope))
        #expect(await store.enqueueCommand(outboxCommand(
            id: "queued-id",
            sessionKey: "queued",
            agentID: "agent-a",
        ...
        let acceptedParked = try #require(await store.loadCommands().first(where: { $0.id == "accepted-id" }))
        #expect(await store.markCommandRetriedIfPresent(
        ...
        let acceptedRetry = try #require(await store.loadCommands().first(where: {
            $0.id == "accepted-retry-id"
        }))
        #expect(acceptedRetry.attemptVersion == 1)

        let sendingParked = try #require(await store.loadCommands().first(where: { $0.id == "sending-id" }))
        #expect(await store.markCommandRetriedIfPresent(
            id: sendingParked.id,
            expectedAttemptVersion: sendingParked.attemptVersion,
            expectedRetryCount: sendingParked.retryCount,
            expectedLastError: sendingParked.lastError,
            agentID: "agent-a",
            deliverySessionKey: "sending",
            routingContract: "per-sender|sending|agent-a",
            replacementID: "sending-retry-id") == .updated)
        let sendingRetry = try #require(await store.loadCommands().first(where: { $0.id == "sending-retry-id" }))
        #expect(sendingRetry.attemptVersion == 1)
    ...
    @Test func `enqueue after branch snapshot is parked by transactional pending check`() async throws {
        let url = try makeDatabaseURL()
        defer { try? FileManager.default.removeItem(at: url.deletingLastPathComponent()) }
        let store = OpenClawChatSQLiteTranscriptCache(databaseURL: url, gatewayID: "gw-a")
        let scope = OpenClawChatOutboxScope(sessionKey: "main", agentID: "agent-a")
        #expect(await store.updateLastActiveLeafEntryID("leaf-a", expectedEpoch: 0, for: scope))
        let previousState = try #require(await store.branchState(for: scope))
        #expect(!previousState.hadPendingCommands)
        ...
        #expect(reconciled.first(where: { $0.id == "racing-enqueue" })?.status == .failed)
        #expect(await store.branchState(for: scope)?.lastActiveLeafEntryID == "leaf-b")
    }
    ...
    @Test func `branch parking wins over a retry captured from the previous failure`() async throws {
        ...
        let initialAttempt = try #require(await parkingStore.claimNextCommand())
        #expect(initialAttempt.id == "c-race")
        #expect(await parkingStore.markCommandFailedIfPresent(
            id: "c-race",
            attemptVersion: initialAttempt.attemptVersion,
            retryCount: 2,
        ...
        #expect(await retryStore.markCommandRetriedIfPresent(
            id: "c-race",
            expectedAttemptVersion: failureBeforeParking.attemptVersion,
    ...
    @Test func `stale delivery callbacks cannot mutate a retried attempt`() async throws {
        ...
        #expect(newAttempt.attemptVersion == 1)

        #expect(await store.markCommandFailedIfPresent(
            id: oldAttempt.id,
            attemptVersion: oldAttempt.attemptVersion,
            retryCount: 3,
            lastError: "stale failure") == .missing)
        #expect(await store.markCommandAwaitingConfirmation(
            id: oldAttempt.id,
            attemptVersion: oldAttempt.attemptVersion) == .missing)
        #expect(await store.confirmCommand(
            id: oldAttempt.id,
            attemptVersion: oldAttempt.attemptVersion) == .missing)

        let current = try #require(await store.loadCommands().first)
        #expect(current.status == .sending)
        #expect(current.attemptVersion == newAttempt.attemptVersion)
        #expect(current.retryCount == newAttempt.retryCount)
        #expect(current.lastError == newAttempt.lastError)
    }
    ...
    @Test func `retry and failure marks persist retry count and last error`() async throws {
        ...
        let firstAttempt = try #require(await store.claimNextCommand())
        #expect(await store.markCommandQueued(
            id: "c-1",
            attemptVersion: firstAttempt.attemptVersion,
            retryCount: 2,
            lastError: "socket closed") == .updated)
        ...
        #expect(loaded.map(\.attemptVersion) == [firstAttempt.attemptVersion + 1])

        let secondAttempt = try #require(await store.claimNextCommand())
        #expect(secondAttempt.id == "c-1")
        #expect(await store.markCommandFailedIfPresent(
            id: "c-1",
            attemptVersion: secondAttempt.attemptVersion,
            retryCount: 3,
