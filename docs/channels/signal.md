...
<Steps>
  ...
  <Step title="Run the guided setup">
    ...
    For non-interactive setup, `openclaw channels add --channel signal` also accepts `--signal-number <e164>` for the bot phone number, plus `--http-host <host>` and `--http-port <port>` for the Signal daemon endpoint (default `127.0.0.1:8080`). To recover several distinct legacy HTTP endpoints atomically, pass `--signal-transports '<json>'` with an account-keyed map such as `{"work":{"kind":"external-native","url":"http://work:8080"},"alerts":{"kind":"container","url":"http://alerts:8080"}}`.
