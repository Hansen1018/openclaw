...
export const ja_JP: TranslationMap = {
  ...
  sessionsView: {
    ...
    changeIcon: "アイコンを変更",
    customEmoji: "カスタム絵文字",
    invalidEmojiIcon: "絵文字をちょうど1つ入力してください。",
    removeIcon: "アイコンを削除",
  ...
  quickSettings: {
    ...
    personal: {
      ...
      saving: "保存中…",
      uploading: "処理中…",
      profileHint: "JPEG、PNG、または WebP。画像は中央でクロップされ、512px にリサイズされます。",
  ...
  subtitles: {
    ...
    apps: "スマートフォン、ウォッチ、デスクトップ、ブラウザ用のコンパニオンアプリ。",
  ...
  custodian: {
    ...
    history: {
      ...
      opaqueChange: "書式やコメントが変更されましたが、表示される構成パスの変更はありません。",
      sources: {
  ...
  mcpServers: {
    ...
    missing: "設定内に MCP サーバー「{name}」が見つかりませんでした。",
  ...
  labsPage: {
    ...
    sectionTitle: "実験的な機能",
    sectionDescription: "変更はすぐに保存され、今後のエージェントの実行に適用されます。",
    ...
    swarm: {
      title: "Swarm",
      description:
        "Code Modeがサブエージェントのグループを並行してオーケストレーションできるようにします。",
  ...
  appsPage: {
    ...
    heroTagline:
      "スマートフォン、時計、デスクトップ、ブラウザ用のコンパニオンアプリ — さらにエージェントの機能を拡張するプラグインも。",
    ...
    cards: {
      ...
      android: {
        title: "Android",
        desc: "Android スマートフォンを完全な OpenClaw ノードとして — チャット、カメラ、Canvas。",
      },
      appleWatch: {
        title: "Apple Watch",
        desc: "手元でチャットをひと目で確認し、素早く返信。",
      },
      wearOs: {
        title: "Wear OS",
        desc: "Android コンパニオンで OpenClaw をあなたの時計に拡張します。",
      },
      macos: {
        title: "macOS",
  ...
  profilePage: {
    ...
    identity: {
      ...
      displayName: "表示名",
      displayNameDescription: "このgatewayを使用する他の人に表示されます。",
  ...
  board: {
    ...
    widget: {
      ...
      appStaleDetail: "サーバー、リソース、または元のトランスクリプトが利用できなくなりました。",
      retry: "再試行",
      frameResolverMissing: "ウィジェットのコンテンツは利用できません。",
      sandboxUnavailable: "ウィジェットサンドボックスのホストが利用できません。",
      frameAuthorizationFailed: "更新を繰り返し試みましたが、ウィジェットの認証に失敗しました。",
      sandboxOriginRequired:
        "更新を繰り返し試みましたが、ウィジェットの認証に失敗しました。ゲートウェイがウィジェットサンドボックスのポートをルーティングしないリバースプロキシまたはトンネルの背後で動作している場合は、mcp.apps.sandboxOrigin をサンドボックスリスナーにルーティングされた専用のパブリックオリジンに設定してください。",
  ...
  chat: {
    ...
    sessionHeader: {
      ...
      copied: "コピーしました",
      branches: "スレッドのブランチ",
      branchSwitchUnavailable: "エージェントの処理中はブランチを切り替えできません。",
    ...
    board: {
      ...
      resetDescription: "会話のコンテキストがリセットされます。ダッシュボードは維持されます。",
    },
    catalog: {
    ...
    messages: {
      ...
      rewindUnavailable: "エージェントの処理中は巻き戻しを利用できません",
      forkUnavailable: "エージェントの処理中はフォークを利用できません",
      unknownDate: "不明な日付",
      voiceNote: "ボイスメモ",
    },
    modelControls: {
    ...
    composer: {
      ...
      cameraInput: "カメラ",
      cameraListUnsupported: "このブラウザーではカメラを一覧表示できません。",
      ...
      holdToRecordSettingDescription:
        "コンポーザーのマイクボタンを押し続けて話し、離すと送信せずにテキストを挿入します。",
      ...
      dictationFinalizationTimedOut: "最後の部分文字起こしが確定される前に音声入力が停止しました。",
